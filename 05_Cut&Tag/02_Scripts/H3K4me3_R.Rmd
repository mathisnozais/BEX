...
H3K4Me3 mark from Jurkat clone (BEX WT) will be compared to 3 others 1D9, 2G5 & 3B6 (that are BEX KO)
...
```{r}
count_mtx<-read.table(file = paste0(WORKING_DIR,"03_Output/H3K4Me3_H3K27Ac_narrow_nomodel/bwa/merged_library/macs3/narrow_peak/consensus/K4me3/K4me3.consensus_peaks.featureCounts.txt"),header = T)
...
names(count_mtx) <- c("1D9_K4Me3","3B6_K4Me3","2G5_K4Me3","JKT_K4Me3") #shorter names
head(count_mtx)
...
saveRDS(dds, file = paste0(WORKING_DIR,"03_Output/dds_K4Me3_BexPos_vs_Bexneg.rds"))
```

# Loading Deseq obj
```{r}
dds <- readRDS(file = paste0(WORKING_DIR,"03_Output/dds_K4Me3_BexPos_vs_Bexneg.rds"))
...
write.table(raw_counts, file=paste0(WORKING_DIR,"03_Output/H3K4me3_raw_counts.txt"),sep = "\t", quote = FALSE, row.names = FALSE) #available in GEO

norm_count <- counts(dds, normalized=TRUE)
norm_count <- cbind(Region_ID = rownames(norm_count), norm_count)
write.table(norm_count, file=paste0(WORKING_DIR,"03_Output/H3K4me3_normalized_counts.txt"),sep = "\t", quote = FALSE, row.names = FALSE) #available in GEO
```
...
write.table(res_sorted_by_padj, file=paste0(WORKING_DIR,"03_Output/dds_H3K4Me3_BexPos_vs_Bexneg_results.txt"),sep = "\t")
```

```{bash}
# ### Bed files for i-cisTarget
# cat 04_Analysis/RNAseq2_analysis/dds_H3K4Me3_BexPos_vs_Bexneg_results.txt | grep -v baseMean | awk '$3 < -1 && $7 < 0.05 {print $1}'| sed s/\"//g | awk -F'_' '{s=""; for(i=3;i<=NF-2;i++) s=s $i OFS; sub(OFS "$","",s); print s" "$(NF-1)" "$NF" "$1"_"$2}' OFS="_" >04_Analysis/RNAseq2_analysis/H3K4Me3_down_Padj-0.05_Log2FC_-1.bed
# cat 04_Analysis/RNAseq2_analysis/dds_H3K4Me3_BexPos_vs_Bexneg_results.txt | grep -v baseMean | awk '$3 > 1 && $7 < 0.05 {print $1}'| sed s/\"//g | awk -F'_' '{s=""; for(i=3;i<=NF-2;i++) s=s $i OFS; sub(OFS "$","",s); print s" "$(NF-1)" "$NF" "$1"_"$2}' OFS="_" >04_Analysis/RNAseq2_analysis/H3K4Me3_up_Padj-0.05_Log2FC_1.bed
```
...
write.table(vst_df, file=paste0(WORKING_DIR,"03_Output/H3K4me3_vst_counts.txt"), sep = "\t", quote = FALSE, row.names = FALSE) #available in GEO
...
column_order <- c("1D9_K4Me3","3B6_K4Me3","2G5_K4Me3","JKT_K4Me3")
