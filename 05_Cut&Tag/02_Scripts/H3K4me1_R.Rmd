...
H3K4Me1 mark from Jurkat clone (BEX WT) will be compared to 3 others 1D9, 2G5 & 3B6 (that are BEX KO)
...
```{r}
count_mtx<-read.table(file = paste0(WORKING_DIR,"/03_Output/H3K4me1_broad_nomodel/bwa/merged_library/macs3/broad_peak/consensus/K4Me1/K4Me1.consensus_peaks.featureCounts.txt"),header = T)
head(count_mtx)
...
names(count_mtx) <- c("1D9_K4Me1","JKT_K4Me1","2G5_K4Me1","3B6_K4Me1")  #shorter names
head(count_mtx) # Check the names order to shorter names below

#prep sample info matrix
condition <- factor(c("BexNeg","BexPos","BexNeg","BexNeg"))
...
#saveRDS(dds, file = paste0(WORKING_DIR,"03_Output/dds_K4Me1_BexPos_vs_Bexneg.rds"))
```

# Loading Deseq obj
```{r}
dds <- readRDS(file = paste0(WORKING_DIR,"03_Output/dds_K4Me1_BexPos_vs_Bexneg.rds"))
...
write.table(raw_counts, file=paste0(WORKING_DIR,"03_Output/H3K4me1_raw_counts.txt"),sep = "\t", quote = FALSE, row.names = FALSE) #available in GEO
norm_count <- counts(dds, normalized=TRUE)
norm_count <- cbind(Region_ID = rownames(norm_count), norm_count)
write.table(norm_count, file=paste0(WORKING_DIR,"03_Output/H3K4me1_normalized_counts.txt"),sep = "\t", quote = FALSE, row.names = FALSE) #available in GEO
```
...
write.table(res_sorted_by_padj, file=paste0(WORKING_DIR,"03_Output/dds_H3K4me1_BexPos_vs_Bexneg_results.txt"),sep = "\t")
```

```{bash}
# ### Bed files for i-cisTarget
# cat 03_Output/dds_H3K4Me1_BexPos_vs_Bexneg_results.txt | grep -v baseMean | awk '$3 < -1 && $7 < 0.05 {print $1}'| sed s/\"//g | awk -F'_' '{s=""; for(i=3;i<=NF-2;i++) s=s $i OFS; sub(OFS "$","",s); print s" "$(NF-1)" "$NF" "$1"_"$2}' OFS="_" >03_Output/H3K4Me1_down_Padj-0.05_Log2FC_1.bed
# cat 03_Output/dds_H3K4Me1_BexPos_vs_Bexneg_results.txt | grep -v baseMean | awk '$3 > 1 && $7 < 0.05 {print $1}'| sed s/\"//g | awk -F'_' '{s=""; for(i=3;i<=NF-2;i++) s=s $i OFS; sub(OFS "$","",s); print s" "$(NF-1)" "$NF" "$1"_"$2}' OFS="_" >03_Output/RNAseq2_analysis/H3K4Me1_up_Padj-0.05_Log2FC_1.bed
```
...
#df[df$regionname %in% c("Interval_629_chr1_203454963_203455927","Interval_29_chr1_3515353_3517678") ,]
...
dim(na.omit(df[df$Significant == 'Significative' & df$log2FoldChange < 1, ]))
rownames(df[df$Significant == 'Significative' & df$log2FoldChange > 1, ])
rownames(df[df$Significant == 'Significative' & df$log2FoldChange < 1, ])
```
...
write.table(vst_df, file=paste0(WORKING_DIR,"03_Output/H3K4me1_vst_counts.txt"), sep = "\t", quote = FALSE, row.names = FALSE) # available in GEO

region_vec <- na.omit(df[df$Significant == "Significative",])$regionname
#region_vec <- c("Bex1","Bex2","Bex3","Bex4","Bex6","Tal1")
#region_vec <- c("Dlx1","Bex1","Smim3","Tal1","Tox2","Trim12a")
...
column_order <- c("JKT_K4Me1","1D9_K4Me1","2G5_K4Me1","3B6_K4Me1")
