...
########################################## STEP 0 #######################################################
####### Modify Variable ######
WORKING_DIR=$(eval echo \$$1) # path to the cloned github
...
unique_names=$(echo "$unique_names" | sed 's/_.*//'|sort|uniq)
...
for name in $unique_names; do
	echo "FastQC $name"
	FILE=$rawdata/${name}_1.fastqc.zip #checking if the output of the tool is existing
	if [ -f "$FILE" ]; then
...
# Trimming
cp /home/$USER/miniconda3/envs/RNAbulk/share/trimmomatic-0.39-2/adapters/NexteraPE-PE.fa .
for name in $unique_names; do
	echo "Trimming $name"
...
for name in $unique_names; do
  ...
  if [ -f "$FILE" ]; then
  	...
  	samtools sort $workdir/02_Preprocessed/Mapping/${name}.bam -o $workdir/02_Preprocessed/Mapping/${name}_sorted.bam
