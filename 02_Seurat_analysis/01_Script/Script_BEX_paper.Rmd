...
counts_matrix <- read.csv(paste0(WORK_DIR,"O2_Seurat_analysis/03_Data/RNA_matrix_sub.csv"), row.names = 1)
metadata <- read.csv(paste0(WORK_DIR,"O2_Seurat_analysis/03_Data/Metadata_matrix_sub.csv"), row.names = 1)
```
...
sub_Seurat_reg <- FindClusters(object = sub_Seurat_reg, 
                          ...
                          verbose = FALSE,
                          random.seed = 1234)
sub_Seurat_reg <- RunUMAP(object = sub_Seurat_reg, reduction = "pca", seed.use = 1234, dims = 1:12)
...
FindMarkers(sub_Seurat_reg, ident.1 = "Tumoral", ident.2 = "Physiological",min.pct = 0.5)
...
tum_phys2 <- tum_phys
#temp <- tum_phys2[tum_phys2$p_val_adj < 10e-70,]
...
#tum_phys3 <- subset(tum_phys2, logpval >80)
#temp2 <- rbind(x=head(tum_phys3,n=10),y=tail(tum_phys3,n=10))



install.packages("forecast")
library(forecast) # used to easily keep the order of my dataframe
library(viridis)
ggplot(temp, aes(x = fct_inorder(gene),fill = -log10(p_val_adj), y = avg_log2FC)) + theme_bw() + geom_bar(stat = "identity")+ theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))+coord_flip() +scale_fill_viridis(option="magma", direction = -1)
#+ scale_fill_gradient(low="blue", high="red")
```

## BEX plot
```{r}
# Old colors : c("gainsboro","moccasin","goldenrod1","orange","red","firebrick4")
# New colors : c(#F1EBE8","E7ADAE","E77CAE","DB54A8","BE2EB9","#7618CA","#5014BA","#230A6A")
